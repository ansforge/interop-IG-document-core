...
* L’instance [example-procedure-data-absent-reason](Procedure-example-procedure-data-absent-reason.md) illustre les deux cas de figure sur le profil [FRProcedureDocument](StructureDefinition-fr-procedure-document.md) : extension `data-absent-reason` pour les liaisons non obligatoires (`code`, `performedDateTime`), et code d’exception natif (`unknown`) pour la liaison `required` (`status`).
